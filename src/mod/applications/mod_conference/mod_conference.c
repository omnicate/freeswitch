...
conference_obj_t *conference_new(char *name, conference_xml_cfg_t cfg, switch_core_session_t *session, switch_memory_pool_t *pool)
{
	...
	if (conference_utils_test_flag(conference, CFLAG_LIVEARRAY_SYNC)) {
		...
		if (strchr(conference->name, '@')) {
			...
			conference->user_event_channel = switch_core_sprintf(conference->pool, "conference-user.%s", conference->name);
		} else {
			...
			conference->user_event_channel = switch_core_sprintf(conference->pool, "conference-user.%s@%s", conference->name, conference->domain);
		}
