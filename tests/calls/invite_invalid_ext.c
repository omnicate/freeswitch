...
void sip_404_response_handler(int err, const struct sip_msg *msg, void *arg);

int main()
{
  char local_ip[64] = {0}, *registrar = NULL, *tmp_buf = NULL, *callid_prefix = "autotest-invite-invalid-ext-";
  ...
  /* Test to confirm that FS returns a 480 if there is no 3pcc and no SDP */
  ...
  /* Test call to invalid extension with valid SDP */
  ...
  err = sip_requestf(&sip_req, sip_stack, 1, "INVITE", to_uri, &re_route, NULL,
		     sip_noop_send_handler, sip_404_response_handler, NULL, invite_mbuf->buf);
