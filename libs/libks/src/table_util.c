 ...
 */
int expand_chars(const void *buf, const int buf_size, char *out, const int out_size)
{
	int buf_c;
	...
	for (buf_c = 0, buf_p = (const unsigned char *) buf;; buf_c++, buf_p++) {
		...
		for (spec_p = (unsigned char *) SPECIAL_CHARS + 1; *(spec_p - 1) != '\0'; spec_p += 2) {
			if (*spec_p == *buf_p) {
				break;
			}
		}
...
static void dump_table(table_t *tab_p)
{
	...
	for (ret = table_first(tab_p, (void **) &key_p, &key_size,
		 ...
		 ret == TABLE_ERROR_NONE; ret = table_next(tab_p, (void **) &key_p, &key_size, (void **) &data_p, &data_size), entry_c++) {
		/* expand the key */
...
int main(int argc, char **argv)
{
	...
	if (ret != TABLE_ERROR_NONE) {
		(void) fprintf(stderr, "table_util: unable to get info on table in '%s': %s\n", in_file, table_strerror(ret));
		exit(1);
	}
	...
	if (num_buckets > 0) {
		...
		if (ret != TABLE_ERROR_NONE) {
			(void) fprintf(stderr, "table_util: unable to adjust table to %d buckets: %s\n", num_buckets, table_strerror(ret));
			exit(1);
		}
		do_write = 1;
	}
